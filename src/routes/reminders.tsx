...
function RemindersPage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    toast.success("Lovely — your reminder is set 🌸", { description: "We'll be here when it's time." });
    ...
    <div className="mx-auto w-full max-w-5xl px-4 py-8 md:px-8 md:py-12">
      <header className="mb-8">
        ...
        <p className="mt-2 text-muted-foreground">
          Add anything that helps you feel cared for. Edit gently, anytime.
        </p>
      </header>

      <form
        onSubmit={submit}
        className="mb-10 rounded-3xl border border-border/60 bg-card/80 p-6 shadow-petal backdrop-blur"
      >
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2">
            ...
            <Input
              ...
              placeholder="e.g. With a small snack, take it slow"
              className="mt-1.5"
            />
          </div>
          <div>
      ...
      <div className="grid gap-3">
        {reminders.map((r) => (
          <div key={r.id} className="group relative">
            ...
            <button
              onClick={() => { remove(r.id); toast("Removed — no judgment, ever 💙"); }}
              aria-label="Remove reminder"
              className="absolute right-3 top-3 z-10 rounded-full bg-card/80 p-1.5 text-muted-foreground opacity-0 backdrop-blur transition hover:text-destructive group-hover:opacity-100"
            >
